...
const PartItem = (props) => {
    ...
    //         diff.hours   = Math.floor(elapsed / 3600);
    //         diff.minutes = Math.floor(elapsed / 60);
    //         diff.seconds = Math.floor(elapsed);

    //         // let wholeTimeElapsed = `${diff.days} days, ${diff.hours} hours, ${diff.minutes} minutes, ${diff.seconds} seconds ago.`;
    //         let message = `${diff.hours} hours ago`;
    ...
    let date = Date.parse(props.parts.date);
        ...
        <ListItem style={{borderBottomWidth:0}}>
            <TouchableWithoutFeedback onPress={() => props.selectPart(props.parts)}>
                <View style={styles.item}>
                    ...
                    <Text style={styles.itemCreatedAt}>{Date(props.parts.date)}</Text>
                </View>
